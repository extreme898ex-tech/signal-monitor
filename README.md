...
## 更新方式を「必要な時だけ」に変更（2026-09-24）
- 毎時の自動実行（cron）をやめました。**Actionsタブ → signal-monitor → Run workflow を押した時だけ**データを取りに行きます。
- 画面（一覧ページ）も勝手に再読込しません。開いたままにしていても通信は発生しません。
- 使い方: 市場が閉じた後など、シグナルを確認したい時に ①Run workflow → ②緑✔を待つ → ③一覧ページを開く（または再読込ボタン）。これだけです。
- 自動実行に戻したい時は `.github/workflows/monitor.yml` の `on:` の下に
  `schedule:\n    - cron: '5 * * * *'` の2行を戻せば元通りになります。

## 銘柄の追加・削除（2026-09-24 追加機能）
銘柄リストは **symbols.csv** ファイルで管理します。GitHubのWeb画面で直接編集できます（ファイルの再アップロード不要）:

1. リポジトリで `symbols.csv` を開く
2. 右上の**鉛筆アイコン**（Edit this file）を押す
3. 行を追加（例: `三井物産,8031.T,日本株,20`）または削除したい行を丸ごと消す
4. 一番下の **Commit changes** を押す
5. Actions → signal-monitor → **Run workflow** で再計算
